...
if __name__ == "__main__":
    ...
    seed = sid.seed
    ###########################################
    # Copy source dir and rename it
    destination_dir = "data/images/image_preprocessing/processed_images/"
    ...
    for folder in os.listdir(source_dir):
        ...
        os.rename(os.path.join(source_dir, folder), os.path.join(source_dir, species_folder))
        if os.path.exists(os.path.join(source_dir, species_folder)):
            if (species_folder != "train") and (species_folder != "val") and (species_folder != "test"):
                # List all files in each species folder
                species_files = os.listdir((os.path.join(source_dir, species_folder)))
                ...
                for file in train_files:
                    ...
                    os.rename(os.path.join(source_dir, species_folder, file), os.path.join(train_dir, species_folder, file))
                for file in val_files:
                    if not os.path.exists(f"{val_dir}/{species_folder}"):
                        os.system(f"mkdir {val_dir}/{species_folder}")
                    os.rename(os.path.join(source_dir, species_folder, file), os.path.join(val_dir, species_folder, file))
                for file in test_files:
                    if not os.path.exists(f"{test_dir}/test"):
                        os.system(f"mkdir {test_dir}/test")
                    os.rename(os.path.join(source_dir, species_folder, file), os.path.join(test_dir, "test", file))      
                # Remove empty folders
                os.rmdir(os.path.join(source_dir, species_folder))
                print(f"{species_folder} files splitted.")
        else:
            print(f"Path not found: {os.path.join(source_dir, folder)}")
